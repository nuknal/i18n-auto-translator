...
const aiBaseURL =
  argv.baseUrl ||
  process.env.TRANSLATOR_AI_BASE_URL ||
...
async function translateWithOpenAI(
  ...
  try {
    ...
    const response = await axios.post(
      path.join(aiBaseURL, 'chat/completions'),
      {
        model: aiModel,
        messages: [
          {
...
- No language indicators like "In French:" or "French translation:"`
    ...
    if (model.startsWith('anthropic.claude')) {
      ...
      throw new Error(`Unsupported Bedrock model: ${aiModel}`)
    }
...
async function findAndTranslateMissingKeys(
  ...
  for (let key in defaultObj) {
    ...
    if (
      ...
      if (!(key in targetObj) || !Array.isArray(targetObj[key])) {
        ...
        for (let i = 0; i < defaultObj[key].length; i++) {
          ...
          if (typeof item === 'object' && !Array.isArray(item)) {
            ...
            filledKeys[key][i] = await translateText(
              localesDir,
              aiApiKey,
              aiModel,
              aiProvider,
      ...
      filledKeys[key] = await translateText(
        localesDir,
        aiApiKey,
        aiModel,
        aiProvider,
